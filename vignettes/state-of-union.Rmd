...
anno$token %>%
  left_join(
    anno$token,
    c("id"="id", "sid"="sid", "tid"="tid_source"),
    suffix=c("", "_source")
  ) %>%
  left_join(anno$document, by="id") %>%
  filter(year == 2001) %>%
  filter(relation == "dobj") %>%
  select(id = id, start = token, word = token_source) %>%
...
anno$token %>%
  left_join(
    anno$token, c("id"="id", "sid"="sid", "tid"="tid_source"),
    suffix=c("", "_source")
  ) %>%
  left_join(anno$document, by="id") %>%
  filter(year == 2002) %>%
...
topic_df <- tibble(topic = as.integer(col(topics)),
                   id = anno$document$id[as.integer(row(topics))],
