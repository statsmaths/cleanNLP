#' Interface for initializingthe coreNLP backend
#'
...
.init_coreNLP_backend <- function() {
  ...
  if (as.numeric(substr(ver, 1, 3)) < 1.8) {
      stop(sprintf("Your Java version (%s) in not sufficently up to date.\n", ver),
  ...
  if (!file.exists(volatiles$coreNLP$lib_location)) {
    stop("Please run download_core_nlp() in order to install required jar files.")
  } else {
