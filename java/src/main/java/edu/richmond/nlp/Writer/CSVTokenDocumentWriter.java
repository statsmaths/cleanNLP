...
public class CSVTokenDocumentWriter {
  ...
  public CSVTokenDocumentWriter(String docID) {
    this.docID = docID;
    this.header = String.format("id,sid,tid,word,lemma,upos,pos,speaker,cid,cid_end%n");
  }
  ...
  public String print(CoreMap sentence) {
    ...
    for (CoreLabel token : tokens) {
      ...
      sb.append(String.format("%s,%d,%d,\"%s\",\"%s\",\"%s\",\"%s\",%s,%s,%s%n", docID,
                              token.sentIndex(), token.index(), word, lemma, upos, pos,
                              speaker, charOffsetStart, charOffsetEnd));
    }
